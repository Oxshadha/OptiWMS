...
public class ForecastResultReadService {
    ...
    public ResponseEntity<Object> getRawMaterialRequirements(String rmSku, String model, String warehouseId) {
        ...
        StringBuilder sql = new StringBuilder("""
                ...
                """);
        if (warehouseId != null && !warehouseId.isBlank())
            sql.append(" AND fr.warehouse_id::text = :warehouseId");
        if (rmSku != null && !rmSku.isBlank())
            sql.append(" AND m.material_code = :rmSku");
        sql.append(
                " GROUP BY m.material_code, m.material_type, i.available_quantity, i.quantity, i.buffer_stock, i.reorder_point ORDER BY m.material_code LIMIT 5000");
        Query q = entityManager.createNativeQuery(sql.toString())
                .setParameter("model", selectedModel);
        if (warehouseId != null && !warehouseId.isBlank())
            q.setParameter("warehouseId", warehouseId);
        if (rmSku != null && !rmSku.isBlank())
            q.setParameter("rmSku", rmSku);
        @SuppressWarnings("unchecked")
        List<Object[]> rows = q.getResultList();
        List<Map<String, Object>> items = rows.stream().map(row -> {
            Map<String, Object> item = new LinkedHashMap<>();
